...
let query = 'SELECT pb.id, pb.case_reference_id, pb.date, pb.doctor_name, p.patient_name, t.amount AS transaction_amount, pb.net_amount,(pb.net_amount - t.amount) AS balance_amount FROM pathology_billing pb JOIN patients p ON pb.patient_id = p.id JOIN transactions t ON pb.transaction_id = t.id join case_references ON pb.case_reference_id = case_references.id'
let values = ['']
