const BloodIssueReportModel = require('../models/bloodIssueReport.model')

exports.GetBloodIssueReport = (req,res)=>{
    let updatedQuery = 'select concat("BLBB",blood_issue.id)as bill_no,blood_issue.date_of_issue,concat(patients.patient_name," ","(",patients.id,")") as receiced_to,blood_bank_products.name as blood_group,patients.gender,blood_donor.donor_name,concat(blood_donor_cycle.bag_no," " ,"(",blood_donor_cycle.volume," ",charge_units.unit,")") as bags,concat(s1.name," ",s1.surname," ","(",s1.employee_id,")") as amount_collectedBy,concat(s2.name," ",s2.surname," ","(",s2.employee_id,")")as blood_collectedBy,blood_issue.net_amount as amount,transactions.amount as paid_amount,(blood_issue.net_amount - transactions.amount) as balance_amount from blood_issue join transactions on transactions.blood_issue_id = blood_issue.id join patients on patients.id = blood_issue.patient_id join blood_donor_cycle on blood_donor_cycle.id = blood_issue.blood_donor_cycle_id join blood_donor on blood_donor.id = blood_donor_cycle.blood_donor_id join blood_bank_products on blood_bank_products.id = blood_donor.blood_bank_product_id join staff as s1 on s1.id = transactions.received_by join staff as s2 on s2.id = blood_issue.generated_by join charge_units on charge_units.id = blood_donor_cycle.unit where blood_issue.id '
    let updatedValues = []
    ...
    if(req.query.bloodCollectedBy){
        updatedQuery +=' and blood_issue.hospital_doctor = ? '
    ...
    if(req.query.bloodGroup){
        updatedQuery += ' and blood_bank_products.id = ?  '
    ...
    BloodIssueReportModel.setQuery(updatedQuery,updatedValues);
    BloodIssueReportModel.GetBloodIssueReport((err,bloodIssueReport)=>{
        if(err)
        res.send(err);

        console.log('Blood issue Report ',bloodIssueReport);
        res.send(bloodIssueReport);
    })
